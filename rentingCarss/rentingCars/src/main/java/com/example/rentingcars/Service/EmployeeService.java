...
public class EmployeeService {
    ...
    public void  CechOCar(Integer emp_id ,Integer car_id,String loc,String con){
        ...
        car.setCondit(con);
        if (car.getCondit().equals("Intact")){
    ...
    public void  promotionEmployee(Integer id){
        ...
        if (employee.getTotalCar()>=90&& employee.getTotalCar()<=180) {
            employee.setRank("advanced");
        }
        if (employee.getTotalCar()>=181&& employee.getTotalCar()<=280) {
            employee.setRank("expert");
        }
    }
